...
public class Bob {
    ...
    public Bob(String circuit_filename, String fmt_filename, String sseed, String sot_type)
		...
		MyUtil.processUpdate(0); 

        MyUtil.initConnection(); 

		System.out.println("Create Connection");
        MyUtil.createConnection();
		
        
        ////Init - create txt if not exist   
        //File file = new File("toAlice.mutex");	
		//if (!file.exists()) {
			//file.createNewFile();
		//}
		//file = new File("toBob.mutex");	
		//if (!file.exists()) {
			//file.createNewFile();
		//}
		//file = new File("buffer.tmp");	
		//if (!file.exists()) {
			//file.createNewFile();
		//}
		...
		////Init mutex files
		//try{			
			//FileOutputStream fos = new FileOutputStream("toAlice.mutex");
			//fos.write(0);				
			//fos.close();			
		//} catch (IOException e) {
			//e.printStackTrace();
		//}
		//try{			
			//FileOutputStream fos = new FileOutputStream("toBob.mutex");
			//fos.write(0);				
			//fos.close();			
		//} catch (IOException e) {
			//e.printStackTrace();
		...
		*/
        ...
        for (i = 0; i < num_of_iterations; i++) {
			...
			System.out.println("Parse IOformat file and prepare inputs");
            try {
                // Preparations
                ...
                //logger.error("Bob: cannot open/close " + fmt_filename + " - " + e.getMessage());
            ...
            // Parse circuit file
			System.out.println("Parse Circuit File");
            try {
                // Preparations
                ...
                //logger.error("Bob: cannot open/close " + circuit_filename + " - " + e.getMessage());
                System.exit(1);
            } catch (Exception e) {
                //logger.error("Bob: exception - " + e.getMessage());
                System.exit(1);
	    ...
	    // ====================
		System.out.println("Start the SFE protocol");
            // Repeatedly encrypt circuit, extract data from it and save
		System.out.println("Send encrypted circuit");
		MyUtil.processUpdate(2); 
	    for (j = 0; j < num_of_circuits; j++) {
               ...
               MyUtil.sendBytes(toAlice, EncPayload, true);              
	    ...
	    // Receive cc_num from Alice
			...
			System.out.println("Send encrypted circuits secrets");
	    for (j = 0; j < num_of_circuits; j++) {
               if (j != cc_num) {
                  SecPayload = (byte[]) vSecPayload.elementAt(j);
                  MyUtil.sendBytes (toAlice, SecPayload, true);                  
	       }
	    }
	    //toAlice.flush();
		...
		System.out.println("Reading Loggitude and Latatude");
		MyUtil.processUpdate(3); 
            f.getBobInput(c, br); 
		...
		System.out.println("Obvious Transfer");	
		MyUtil.processUpdate(4);
            OT.SenderOTs(c, f, ot, toAlice, fromAlice);
			
		
	    // Get Bob garbled results from Alice & print them
		System.out.println("Get garbled results");	
	...
	//public static void tm(String[] args) {
        String filename;
        String circ_fname;
        String fmt_fname;
	boolean edit = false;
	...
	if (compile) {
         ...
         if (run) {
            ...
            try {
				MyUtil.sender_id = Integer.parseInt(args[4]);
				MyUtil.receiver_id = Integer.parseInt(args[5]);
