...
public class MyUtil {
    ...
    public static BigInteger EG_encrypt(BigInteger y, BigInteger r,
        ...
        //logger.debug("EG_encrypt encrypting: " + MyUtil.toHexString(padded_mess));
    ...
    */
    ...
    public static void sendOTMESS(ObjectOutputStream oos, OTMESS otmess) {
        try {
            ...
            String encoded = Base32.encode(baos.toByteArray());
           	
           	String message = ""; 
			do{
				if(message.equals( dataStream.failMessage())){
					requestWait();
				}
				
				
				//DEBUG
				//System.out.println("sendOTMESS: "+ encoded);	
				System.out.println("  - sendOTMESS String length: "+ encoded.length());	
				
				message = dataStream.postData(sender_id, receiver_id, encoded);
				
			} while (message.equals( dataStream.failMessage()));
    ...
    public static OTMESS receiveOTMESS(ObjectInputStream ois) {
        ...
        try {
			...
			do{
				...
				message = dataStream.getData(sender_id, receiver_id);
			} while (message.equals( dataStream.failMessage()));
			//DEBUG
			//System.out.println("receiveOTMESS: "+ message);	
			System.out.println("  - receiveOTMESS String length: "+ message.length());	
				
            byte[] decoded = Base32.decode(message);
    ...
    public static void sendInt(ObjectOutputStream oos, int k, boolean flush) {
        try {
			...
			do{
				...
				message = dataStream.postData(sender_id, receiver_id, Integer.toString(k));
			} while (message.equals( dataStream.failMessage()));
			//DEBUG
			System.out.println("  - sendInt: "+ k);	
    ...
    public static int receiveInt(ObjectInputStream ois) {
        ...
        try {
			...
			//FileInputStream fis = new FileInputStream("buffer.tmp");
			...
			//DEBUG
			System.out.println("  - receiveInt: "+ k);	
    ...
    public static void sendBytes (ObjectOutputStream oos, byte[] a, boolean flush) {
        try {
			...
			//FileOutputStream fos = new FileOutputStream("buffer.tmp");			
            ...
            //fos.flush();
			//fos.close();
			String encoded = Base32.encode(a);
			...
			do{
				...
				//DEBUG
				//System.out.println("sendBytes: "+ encoded);	
				System.out.println("  - sendBytes String length: "+ encoded.length());	
    ...
    public static void receiveBytes(ObjectInputStream ois, byte[] in, int len) {
        ...
        try {
			...
			//DEBUG
			//System.out.println("receiveBytes: "+ message);	
			System.out.println("  - receiveBytes String length: "+ message.length());	
							
            byte[] tmpArr = Base32.decode(message);
    ...
    /**
     * upMutex - set toAlice.mutex or toBob.mutex to 1 depends on the caller
     * 
     */
    public static void upMutex() {
		try{	
			...
			do {				
				...
				if (className.equals("Bob")){
					//fos = new FileOutputStream("toAlice.mutex");
					message = dataStream.postToReceiverMutex(sender_id, receiver_id, 1);
				} else {					
					//fos = new FileOutputStream("toBob.mutex");
					message = dataStream.postToSenderMutex(sender_id, receiver_id, 1);
				}	
			} while (message.equals( dataStream.failMessage()));
			System.out.println("  - upMutex()");
			//fos.write(0);	
			//fos.flush();				
			//fos.close();			
		} catch (Exception e) {
	...
	/**
     * downMutex - set toAlice.mutex or toBob.mutex to 0 depends on the caller
     * 
     */
	public static void downMutex() {
		try{			
			...
			do {				
				...
				if (className.equals("Bob")){
					//fos = new FileOutputStream("toBob.mutex");
					message = dataStream.postToSenderMutex(sender_id, receiver_id, 0);
				} else {
					//fos = new FileOutputStream("toAlice.mutex");
					message = dataStream.postToReceiverMutex(sender_id, receiver_id, 0);
				}	
			} while (message.equals( dataStream.failMessage()));
			System.out.println("  - downMutex()");
			//fos.write(0);	
			//fos.flush();				
			//fos.close();			
		} catch (Exception e) {
	...
	/**
     * getReceiveMutex - Get mutex value for receiver
     * 
     * @return  value of toAlice.mutex or toBob.mutex depends on receiver
     */
	public static int getReceiveMutex() {
		try{
			StackTraceElement[] stackTraceElements = Thread.currentThread().getStackTrace();
			String className = stackTraceElements[stackTraceElements.length-1].getClassName();			
			...
			do {				
				...
				if (className.equals("Bob")){
					//fis = new FileInputStream("toBob.mutex");
					message = dataStream.getToSenderMutex(sender_id, receiver_id);		
				} else {					
					//fis = new FileInputStream("toAlice.mutex");	
					message = dataStream.getToReceiverMutex(sender_id, receiver_id);				
				}		
			} while (message.equals( dataStream.failMessage()));
			//System.out.println("getReceiveMutex() message: "+ message);
			//int tmp = fis.read();		
			int tmp = Integer.parseInt(message);
	...
	/**
     * getSendMutex - Get mutex value for sender
     * 
     * @return  value of toAlice.mutex or toBob.mutex depends on sender
     */
	public static int getSendMutex() {
		try{
			StackTraceElement[] stackTraceElements = Thread.currentThread().getStackTrace();
			String className = stackTraceElements[stackTraceElements.length-1].getClassName();
			//FileInputStream fis = null;
			...
			//System.out.println("getSendMutex() message: "+ message);	
			//int tmp = fis.read();		
			int tmp = Integer.parseInt(message);
	...
	/**
     * receiveWait - wait until getReceiveMutex() == 1 to receive data 
     *   
     */
	public static void receiveWait(){
		int state = MyUtil.getReceiveMutex();
        while (state != 1){
			try {
				Thread.sleep(1000);
			...
			state = MyUtil.getReceiveMutex();
		}		
	}
	
	/**
     * sendWait - wait until getSendMutex() == 0 to send data   
     * 
     */
	public static void sendWait(){
		int state = MyUtil.getSendMutex();
        while (state != 0){
			try {
				Thread.sleep(1000);
			} catch(InterruptedException ex) {
			...
			state = MyUtil.getSendMutex();
		}		
	}
	
	/**
     * requestWait() - sleep for a period of time
     * 
     */
	...
	public static void createConnection(){
		dataStream.createConnection(sender_id, receiver_id);
	}
	
	public static void initConnection(){
		dataStream.initConnection(sender_id, receiver_id);
	}
	
	public static void processUpdate(int state){
		dataStream.processUpdate(sender_id, receiver_id, state);
	}
